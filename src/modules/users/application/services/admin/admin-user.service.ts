...
import {
    ...
    toAdminUserListItem,
    toAdminUserSession,
} from '@/modules/users/application/utils/admin-user-mapper';
...
export class AdminUserService {
    ...
    async updateRole(
        ...
        if (!prepared) {
            ...
            // Cho phép retry mirror Keycloak khi DB đã đúng role nhưng lần trước external sync thất bại.
        ...
        await this.publishRoleChangedEmail(prepared, dto.role, actorId);
        if (syncStatus === 'FAILED')
            throw new ServiceUnavailableException(
                'Role đã lưu nhưng chưa đồng bộ Keycloak; hãy retry.',
            );
    ...
    async updateStatus(
        ...
        await this.finishAudit(prepared.auditId, syncStatus, targetId);
        await this.publishStatusChangedEmail(prepared, dto.status, actorId);
        if (syncStatus === 'FAILED')
            throw new ServiceUnavailableException(
                'Trạng thái đã lưu nhưng chưa đồng bộ Keycloak; hãy retry.',
            );
    ...
    private async prepareRoleMutation(
        ...
        return { user, auditId: audit.id, before, reason: dto.reason };
    }
    ...
    // Phát email sau khi local status đã commit và audit đã được cập nhật để người dùng luôn nhận
    // thông báo phản ánh nguồn quyền động của Auth Service. Kafka/SMTP là side effect bất đồng bộ,
    // vì vậy lỗi gửi chỉ được log; không rollback trạng thái đã được admin xác nhận trong DB.
    private async publishStatusChangedEmail(
        prepared: PreparedMutation,
        status: UserStatus,
        actorId: string,
    ): Promise<void> {
        const previousStatus = prepared.before.status as UserStatus;
        if (previousStatus === status) return;

        const event: UserStatusChangedEvent = {
            eventId: prepared.auditId,
            eventName: UserEvents.STATUS_CHANGED,
            eventVersion: 1,
            source: 'auth-service',
            occurredAt: new Date().toISOString(),
            aggregateId: prepared.user.id,
            metadata: { actorUserId: actorId, causationId: prepared.auditId },
            data: {
        ...
        await this.kafkaProducer.publish(UserEvents.STATUS_CHANGED, event);
    }

    // Phát email sau khi role local đã commit và audit đã cập nhật; mọi session đã bị revoke trong
    // transaction trước đó nên email có thể thông báo luôn việc user cần đăng nhập lại để nhận quyền mới.
    private async publishRoleChangedEmail(
        prepared: PreparedMutation,
        role: UserRole,
        actorId: string,
    ): Promise<void> {
        const previousRole = prepared.before.role as UserRole;
        if (previousRole === role) return;

        const event: UserRoleChangedEvent = {
            eventId: prepared.auditId,
            eventName: UserEvents.ROLE_CHANGED,
            eventVersion: 1,
            source: 'auth-service',
            occurredAt: new Date().toISOString(),
            aggregateId: prepared.user.id,
        ...
        await this.kafkaProducer.publish(UserEvents.ROLE_CHANGED, event);
    }
    ...
    private async revokeAllSessionsWithManager(
        ...
        const result = await manager
            ...
            .where(
                'userId = :userId AND revokedAt IS NULL AND expiresAt > :now',
                {
                    userId,
                    now: new Date(),
                },
    ...
    private async syncKeycloakStatus(
        user: User,
    ): Promise<'SUCCESS' | 'FAILED'> {
        try {
            await this.keycloakAdmin.syncUserStatus(
                user.keycloakId,
                user.status,
            );
            return 'SUCCESS';
