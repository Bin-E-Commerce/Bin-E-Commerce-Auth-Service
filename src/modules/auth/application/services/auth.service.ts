...
import {
    TokenService,
    TokenPair,
} from '@/modules/auth/application/services/token.service';
import { RegisterInitiateDto } from '@/modules/auth/presentation/dto/register-initiate.dto';
import { RegisterVerifyDto } from '@/modules/auth/presentation/dto/register-verify.dto';
import { LoginDto } from '@/modules/auth/presentation/dto/login.dto';
import { SocialCallbackDto } from '@/modules/auth/presentation/dto/social-callback.dto';
import { ForgotPasswordDto } from '@/modules/auth/presentation/dto/forgot-password.dto';
...
import {
    AuthResponse,
    AuthUserResponse,
} from '@/modules/auth/presentation/dto/auth-response.dto';
import { SessionResponseDto } from '@/modules/users/presentation/dto/session-response.dto';
...
export class AuthService {
    ...
    private async syncKeycloakIdentity(
        ...
        if (
            !subject ||
            (tokenEmail && tokenEmail !== user.email.toLowerCase())
        ) {
            throw new UnauthorizedException('Invalid credentials');
        }
    ...
    async changePassword(
        ...
        try {
            await this.tokenService.revokeToken(
                verificationTokens.refreshToken,
            );
        } catch {
    ...
    private toSessionResponse(
        ...
        return {
            ...
            loginMethod:
                session.loginMethod ??
                (session.clientId ? 'google' : 'password'),
    ...
    private async toAuthUser(
        ...
        const { id, email, name, phone, role, status, avatarUrl, createdAt } =
            user;
        const shopLogoUrl = await this.sellerShopClient.getShopLogoUrl(id);
